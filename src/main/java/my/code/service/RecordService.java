...
public class RecordService {
    ...
    public RecordsContainerDto findAllRecords(String filterMode) {
        ...
        int numberOfDoneRecords = (int) records.stream().filter(record -> record.getStatus() == RecordStatus.DONE)
                .count();

        int numberOfActiveRecords = (int) records.stream().filter(record -> record.getStatus() == RecordStatus.ACTIVE)
                .count();
